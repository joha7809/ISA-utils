...
fn test_multiple_labels_same_location() {
    ...
    assert!(result.is_ok());
    let instructions = result.unwrap();
    assert_eq!(instructions.len(), 2); // Both labels point to same instruction
}
...
fn test_operand_type_mismatch() {
    let input = "ADD R1, 100, R3
    END"; // ADD needs three registers, not reg+imm+reg
...
fn test_all_r3_instructions() {
    ...
    assert_eq!(instructions[5].opcode, Opcode::END);
}
...
fn test_comments_ignored() {
    ...
    assert!(result.is_ok());
    let instructions = result.unwrap();
    assert_eq!(instructions.len(), 3); // Only three instructions
}
